...
//      DKV.put(tfr);
    ...
    System.out.flush();
    for( double mse : mses )
      assertEquals(0.21693525517287557, mse, 1e-8); //check for the same result on 1 nodes and 5 nodes (will only work with enough chunks), mse, 1e-8); //check for the same result on 1 nodes and 5 nodes (will only work with enough chunks)
  }
...
//      DKV.put(tfr);
    ...
    for(double d:mses)
      System.out.println(d);
    for( double mse : mses )
      assertEquals(0.21693525517287557, mse, 1e-8); //check for the same result on 1 nodes and 5 nodes (will only work with enough chunks)
  }
  ...
  public void testNFoldAirline() {
    ...
    try {
      ...
      ModelMetricsBinomial mm = (ModelMetricsBinomial)gbm._output._cross_validation_metrics;
      assertEquals(0.730441773442826, mm.auc_obj()._auc, 1e-4); // 1 node
      assertEquals(0.22517611872723386, mm.mse(), 1e-4);
      assertEquals(0.6426779746269872, mm.logloss(), 1e-4);
