...
def glrm_pubdev_3728_arrest():
  ...
  try:
    ...
    print(e)
    if sys.version_info < (3,0,0):  # use for python version 2.
        assert "Cannot use init mode SVD when using binary" in e.message.exception_msg, \
            "Should have raised error concerning using init mode SVD with GLRM, binary loss with binary categorical " \
            "columns."
    else:
        assert "Cannot use init mode SVD when using binary" in e.args[0]._props["exception_msg"], \
            "Should have raised error concerning using init mode SVD with GLRM, binary loss with binary categorical " \
            "columns."
  ...
  assert pyunit_utils.equal_two_arrays(glrm_h2o._model_json["output"]["singular_vals"],
                                      glrm_h2o._model_json["output"]["singular_vals"], 1e-6, 1e-4), \
